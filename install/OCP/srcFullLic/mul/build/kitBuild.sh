...
a_distGenTar()
{
  ...
  tar cf ${tarFileFullName} .

  ls -l ${tarFileFullName}

  #tar tvf ${tarFileFullName}
...
_EOF_
  ...
  cat > MsgFile.txt << _EOF_
Thank you for installing Neda Communications, Inc's  ESROS  ${RELID_PROD_NAME}-${RELID_REVNUM} For Windows-CE (${targetArchType}).
...
a_preDistBuild()
{
  ...
  # /dos/u/neda/devenv/esrosFull.mul/relnotes/esros-srcFull-mul-lic-gold.tty

  if [ -r ${CURENVBASE}/relnotes/esros-srcFull-mul-lic-gold.tty ]
    then
      cp ${CURENVBASE}/relnotes/esros-srcFull-mul-lic-gold.tty ${prodDataBaseDir}/relnotes
    else
      print -r -- "${CURENVBASE}/relnotes/esros-srcFull-mul-lic-gold.tty was not found"
      exit 1
    fi

  # NOTYET -- not a good way
  #cp -r ${prodDataBaseDir}/* ${preDistBaseDir}

  cd ${prodDataBaseDir}
  find . -print | cpio -ocB | (cd ../${preDistBaseDIr}; cpio -icBdmu)
...
a_baseFilesListMake()
{
  ...
  find . -print | \
   ...
   egrep -v '/CVS' | \
   egrep -v './install|/relnotes|README' 
}
