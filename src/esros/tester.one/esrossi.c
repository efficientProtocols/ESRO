 ...
 * File name: esrossi.c
 *
 * Description: Scenario Interpreter parser.
 *
-*/
...
*/
		...
		UI_printf("Enter scenario file name (or 'bye' to exit): ");
#ifdef MSDOS
		OS_isrActive = 1;
#endif
		OS_EnterAtomic();
...
Void
log(char *a, int b, int c, int d, int e, int f, int g, int h)
{
	if (verbose) {
...
Void
plog(char *a, int b, int c, int d, int e, int f, int g, int h)
{
	if (verbose)
...
Void
fatal(char *a, int b, int c, int d, int e, int f)
{
