...
#ifdef RCS_VER	/*{*/
static char rcs[] = "$Id: extfuncs.h,v 1.2 2002/10/25 19:37:46 mohsen Exp $";
#endif /*}*/
...
int failureind(int failureValue, unsigned long int userInvokeRef);
int errorconf(unsigned long int userInvokeRef);
int resultconf(unsigned long int userInvokeRef);
int resultind(int encodingType, char *value, unsigned long int userInvokeRef);
